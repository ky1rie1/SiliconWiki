...
| **阶段 3** | **自选装机配置器与兼容性检查** | **实施中** | 我的装机单、独立纯函数五态兼容性评估、可解释替代建议、预算功耗与导入导出（详见 docs/PHASE_3_PLAN.md） |
...
## 阶段 3：自选装机配置器与兼容性检查（实施中）

详细架构规范、五态兼容性判定矩阵与数据契约已整理保存在 [`docs/PHASE_3_PLAN.md`](./PHASE_3_PLAN.md)。

### 1. 本阶段核心规划与设计基准
- **五态独立判定系统**：`pass` / `warning` / `error` / `unknown` / `not-applicable`。`unknown` 代表信息不全，绝对不合并为 warning，亦不判定为整机兼容；
- **集中化规格适配层 (`specAdapter`)**：以阶段 2 `HardwareRecord` 结构化目录为主入口，保留字段来源、单位、核验状态与适用条件；
- **纯函数解耦引擎 (`pcCompatibility.ts`)**：覆盖插槽、BIOS、内存条数与形态、板型机箱匹配、显卡限长、散热器高度/冷排、必要供电引脚及电源容量；
- **可解释沙盒替代建议**：候选配件通过整机克隆复核，确认消解原冲突且不引入新硬冲突后推荐；
- **装机单与持久化**：`schemaVersion: 1`，区分自定义 ¥0 与未报价，导入采用“校验 → 预览 → 应用”；URL 分享仅包含必要配置信息并保护 Base64 大小写，绝不自动覆盖本地已有草稿。

### 2. 推进路线
- **3A**：数据契约、规格提取适配层、兼容性引擎与成本纯函数；
- **3B**：装机单界面、配件选择器、诊断报告与本地持久化；
- **3C**：导入导出、URL 分享、路由集成与生产交互回归。
