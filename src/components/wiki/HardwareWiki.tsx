...
export const HardwareWiki: React.FC<HardwareWikiProps> = ({ onNavigateToGlossary }) => {
  ...
  const categories: { id: HardwareCategory | 'all'; label: string; icon: React.ReactNode }[] = [
    { id: 'all', label: lang === 'en' ? 'All Components' : (t('catAll') || '全部分类'), icon: <Layers className="w-4 h-4" /> },
    { id: 'cpu', label: lang === 'en' ? 'Processors (CPU)' : (t('catCpu') || '处理器 (CPU)'), icon: <Cpu className="w-4 h-4" /> },
    { id: 'gpu', label: lang === 'en' ? 'Graphics Cards (GPU)' : (t('catGpu') || '独立显卡 (GPU)'), icon: <Tv className="w-4 h-4" /> },
    { id: 'laptop', label: lang === 'en' ? '💻 Laptop Guides' : (t('catLaptop') || '笔记本 (Laptop)'), icon: <Laptop className="w-4 h-4" /> },
    { id: 'motherboard', label: lang === 'en' ? 'Motherboards' : (t('catMotherboard') || '主板 (MB)'), icon: <Zap className="w-4 h-4" /> },
    { id: 'ram', label: lang === 'en' ? 'Memory (RAM)' : (t('catRam') || '内存 (RAM)'), icon: <Zap className="w-4 h-4" /> },
    { id: 'storage', label: lang === 'en' ? 'Storage (SSD)' : (t('catStorage') || '固态硬盘 (SSD)'), icon: <HardDrive className="w-4 h-4" /> },
    { id: 'psu', label: lang === 'en' ? 'Power Supplies (PSU)' : (t('catPsu') || '电源 (PSU)'), icon: <Zap className="w-4 h-4" /> },
    { id: 'cooler', label: lang === 'en' ? 'Cooling (Cooler)' : (t('catCooler') || '散热器 (Cooler)'), icon: <Flame className="w-4 h-4" /> },
    { id: 'case', label: lang === 'en' ? 'Chassis (Case)' : (t('catCase') || '机箱 (Case)'), icon: <Box className="w-4 h-4" /> },
  ];
  ...
  const brandListWithCounts = useMemo(() => {
    ...
    return [
      { name: 'all', label: lang === 'en' ? 'All Brands' : (t('allBrands') || '全部品牌'), count: baseCategoryPool.length },
      ...sortedBrands.map((b) => ({ name: b, label: b, count: counts[b] })),
    ];
  }, [baseCategoryPool, t, lang]);
    ...
    switch (selectedCategory) {
      case 'cpu':
        return [
          {
            id: 'socket',
            label: lang === 'en' ? 'Socket Platform' : '平台插槽',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Platforms' : '全部平台', matcher: () => true },
              {
                id: 'am5',
                label: lang === 'en' ? 'AMD AM5 (9000/7000 Series)' : 'AMD AM5 (9000/7000系列)',
                matcher: (i) => {
                  const socket = (
                    (i.specs['插槽'] || '') +
                    ' ' +
              ...
              {
                id: 'lga1851',
                label: lang === 'en' ? 'Intel LGA1851 (Ultra 200S)' : 'Intel LGA1851 (Ultra 200S)',
                matcher: (i) => {
                  const socket = (
                    (i.specs['插槽'] || '') +
                    ' ' +
              ...
              {
                id: 'lga1700',
                label: lang === 'en' ? 'Intel LGA1700 (14/13/12th Gen)' : 'Intel LGA1700 (14/13/12代)',
                matcher: (i) => {
                  const socket = (
                    (i.specs['插槽'] || '') +
                    ' ' +
              ...
              {
                id: 'am4',
                label: lang === 'en' ? 'AMD AM4 (5000 Budget)' : 'AMD AM4 (5000性价比)',
                matcher: (i) => {
                  const socket = (
                    (i.specs['插槽'] || '') +
                    ' ' +
          ...
          },
          {
            ...
            options: [
              ...
              {
                id: 'oc',
                label: lang === 'en' ? 'Overclockable (K/KF/X)' : '可超频 (K/KF/X)',
                matcher: (i) =>
              ...
              {
                id: 'f-series',
                label: lang === 'en' ? 'Value No iGPU (F-Series)' : '甜点无核显 (F系列)',
                matcher: (i) =>
                  /\d+[Ff]\b/.test(i.name) ||
              ...
              {
                id: 'high-core',
                label: lang === 'en' ? 'Multi-Core Productivity (14+ Cores)' : '多核生产力 (14核+)',
                matcher: (i) => {
                  const raw =
                    (i.specs['核心/线程'] || '') +
                    ' ' +
      ...
      case 'gpu':
        return [
          {
            id: 'series',
            label: lang === 'en' ? 'GPU Architecture' : '架构代系',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Generations' : '全部代系', matcher: () => true },
              {
                id: 'rtx50',
                label: lang === 'en' ? 'NVIDIA RTX 50 Series (Blackwell)' : 'NVIDIA RTX 50 系列 (Blackwell)',
                matcher: (i) =>
              ...
              {
                id: 'rtx40',
                label: lang === 'en' ? 'NVIDIA RTX 40 Series (Ada)' : 'NVIDIA RTX 40 系列 (Ada)',
                matcher: (i) =>
              ...
              {
                id: 'rx7000',
                label: lang === 'en' ? 'AMD RX 7000 Series (RDNA 3)' : 'AMD RX 7000 系列 (RDNA 3)',
                matcher: (i) =>
                  i.brand === 'AMD' ||
          ...
          {
            id: 'vram',
            label: lang === 'en' ? 'VRAM Capacity' : '显存规格',
            options: [
              { id: 'all', label: lang === 'en' ? 'All VRAM' : '全部显存', matcher: () => true },
              {
                id: 'vram16g',
                label: lang === 'en' ? '16GB+ (4K / AI Workloads)' : '16GB及以上 (4K/AI大显存)',
                matcher: (i) => {
                  const v =
                    (i.specs['显存容量/类型'] || '') +
                    ' ' +
              ...
              {
                id: 'vram12g',
                label: lang === 'en' ? '12GB Mainstream Sweetspot' : '12GB 甜点主流',
                matcher: (i) => {
                  const v =
                    (i.specs['显存容量/类型'] || '') +
                    ' ' +
              ...
              {
                id: 'vram8g',
                label: lang === 'en' ? '8GB Entry / Budget' : '8GB 高性价比',
                matcher: (i) => {
                  const v =
                    (i.specs['显存容量/类型'] || '') +
                    ' ' +
      ...
      case 'motherboard':
        return [
          {
            id: 'formFactor',
            label: lang === 'en' ? 'Form Factor' : '主板板型',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Form Factors' : '全部板型', matcher: () => true },
              {
                id: 'matx',
                label: lang === 'en' ? 'M-ATX Compact Mainstream' : 'M-ATX 紧凑主流',
                matcher: (i) => {
                  const ff = (
                    (i.specs['主板板型'] || '') +
                    ' ' +
              ...
              {
                id: 'atx',
                label: lang === 'en' ? 'ATX Standard Full Size' : 'ATX 标准大板',
                matcher: (i) => {
                  const ff = (
                    (i.specs['主板板型'] || '') +
                    ' ' +
              ...
              {
                id: 'itx',
                label: lang === 'en' ? 'ITX SFF Mini' : 'ITX 迷你钢炮',
                matcher: (i) => {
                  const ff = (
                    (i.specs['主板板型'] || '') +
                    ' ' +
          ...
          {
            id: 'chipset',
            label: lang === 'en' ? 'Chipset Platform' : '芯片组平台',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Chipsets' : '全部芯片组', matcher: () => true },
              {
                id: 'amd-am5',
                label: lang === 'en' ? 'AMD AM5 (B650 / X870)' : 'AMD AM5 (B650 / X870)',
                matcher: (i) => {
                  const text = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'intel-lga1700',
                label: lang === 'en' ? 'Intel LGA1700 (B760 / Z790)' : 'Intel LGA1700 (B760 / Z790)',
                matcher: (i) => {
                  const text = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'intel-lga1851',
                label: lang === 'en' ? 'Intel LGA1851 (Z890 / B860)' : 'Intel LGA1851 (Z890 / B860)',
                matcher: (i) => {
                  const text = (
                    i.name +
                    ' ' +
          ...
          {
            id: 'feature',
            label: lang === 'en' ? 'Special Features' : '特色设计',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Designs' : '全部设计', matcher: () => true },
              {
                id: 'pcie5',
                label: lang === 'en' ? 'Native PCIe 5.0 M.2' : '原生 PCIe 5.0 M.2',
                matcher: (i) =>
              ...
              {
                id: 'white',
                label: lang === 'en' ? 'White / Silver Heatsink' : '纯白/银白马甲',
                matcher: (i) =>
                  i.name.includes('白色') ||
                  i.name.includes('吹雪') ||
                  i.name.includes('天选') ||
              ...
              {
                id: 'wifi',
                label: lang === 'en' ? 'Built-in Wi-Fi' : '标配 Wi-Fi 无线网卡',
                matcher: (i) =>
      ...
      case 'ram':
        return [
          {
            id: 'gen',
            label: lang === 'en' ? 'Memory Generation' : '内存代系',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Generations' : '全部代系', matcher: () => true },
              {
                id: 'ddr5',
                label: lang === 'en' ? 'DDR5 Next-Gen Mainstream' : 'DDR5 新一代主流',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'ddr4',
                label: lang === 'en' ? 'DDR4 Classic Budget' : 'DDR4 经典性价比',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
          ...
          {
            id: 'freq',
            label: lang === 'en' ? 'Rated Frequency' : '标称频率',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Frequencies' : '全部频率', matcher: () => true },
              {
                id: 'f6000',
                label: lang === 'en' ? '6000~6400 MT/s Sweetspot' : '6000~6400 MT/s 甜点',
                matcher: (i) => {
              ...
              {
                id: 'f6800',
                label: lang === 'en' ? '6800~7200+ MT/s High-Speed' : '6800~7200+ MT/s 极速特挑',
                matcher: (i) => {
                  const s = i.name + ' ' + (i.specs['标称频率'] || '') + ' ' + (i.specs['频率'] || '');
                  return (
                    s.includes('6800') ||
              ...
              {
                id: 'f3200',
                label: lang === 'en' ? '3200~3600 MT/s DDR4' : '3200~3600 MT/s DDR4',
                matcher: (i) => {
                  const s = i.name + ' ' + (i.specs['标称频率'] || '') + ' ' + (i.specs['频率'] || '');
          ...
          {
            id: 'rgb',
            label: lang === 'en' ? 'Lighting & Aesthetics' : '灯效外观',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Appearances' : '全部外观', matcher: () => true },
              {
                id: 'rgb',
                label: lang === 'en' ? 'ARGB Sync Lighting' : 'ARGB 神光同步',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'no-rgb',
                label: lang === 'en' ? 'Stealth Non-RGB Metal' : '无光金属低调马甲',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
      ...
      case 'storage':
        return [
          {
            id: 'bus',
            label: lang === 'en' ? 'Interface Protocol' : '接口协议',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Protocols' : '全部协议', matcher: () => true },
              {
                id: 'pcie4',
                label: lang === 'en' ? 'PCIe 4.0 Full Speed (7000MB/s+)' : 'PCIe 4.0 满速 (7000MB/s+)',
                matcher: (i) => {
                  const s = (
                    (i.specs['接口总线'] || '') +
                    ' ' +
              ...
              {
                id: 'pcie5',
                label: lang === 'en' ? 'PCIe 5.0 Flagship (10000MB/s+)' : 'PCIe 5.0 旗舰 (10000MB/s+)',
                matcher: (i) => {
                  const s = (
                    (i.specs['接口总线'] || '') +
                    ' ' +
          ...
          {
            id: 'capacity',
            label: lang === 'en' ? 'Capacity' : '容量规格',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Capacities' : '全部容量', matcher: () => true },
              {
                id: 'cap2tb',
                label: lang === 'en' ? '2TB+ High Capacity' : '2TB 及以上大容量',
                matcher: (i) => {
              ...
              {
                id: 'cap1tb',
                label: lang === 'en' ? '1TB Mainstream Sweetspot' : '1TB 主流甜点',
                matcher: (i) => {
                  const s = (i.name + ' ' + (i.specs['容量'] || '')).toUpperCase();
                  return (
          ...
          {
            id: 'dram',
            label: lang === 'en' ? 'DRAM Cache' : '缓存架构',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Architectures' : '全部架构', matcher: () => true },
              {
                id: 'dram-yes',
                label: lang === 'en' ? 'Dedicated Physical DRAM' : '独立物理 DRAM 缓存',
                matcher: (i) => {
                  const c =
                    (i.specs['独立缓存 (DRAM)'] || '') +
                    ' ' +
              ...
              {
                id: 'hmb',
                label: lang === 'en' ? 'HMB DRAM-less Value' : 'HMB 高性价比无缓',
                matcher: (i) => {
                  const c =
                    (i.specs['独立缓存 (DRAM)'] || '') +
                    ' ' +
      ...
      case 'cooler':
        return [
          {
            id: 'type',
            label: lang === 'en' ? 'Cooler Type' : '散热形态',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Types' : '全部形态', matcher: () => true },
              {
                id: 'aio360',
                label: lang === 'en' ? '360mm AIO Liquid' : '360 一体水冷',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'aio240',
                label: lang === 'en' ? '240mm AIO Liquid' : '240 一体水冷',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'air-dual',
                label: lang === 'en' ? 'Dual-Tower Dual-Fan Air' : '双塔双扇顶级风冷',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'air-single',
                label: lang === 'en' ? 'Single-Tower 4-Heatpipe Air' : '单塔四热管风冷',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
          ...
          {
            id: 'tdp',
            label: lang === 'en' ? 'Cooling Capacity' : '解热能力',
            options: [
              { id: 'all', label: lang === 'en' ? 'All TDP Ratings' : '全部解热功耗', matcher: () => true },
              {
                id: 'tdp250',
                label: lang === 'en' ? '250W+ Flagship (i9/9950X)' : '250W+ 旗舰压制 (i9/9950X)',
                matcher: (i) => {
                  const raw =
              ...
              {
                id: 'tdp200',
                label: lang === 'en' ? '200W~240W Sweetspot' : '200W~240W 甜点压制',
                matcher: (i) => {
                  const raw =
                    i.specs['标称解热功耗 (D-TDP)'] ||
                    i.specs['解热能力 (D-TDP)'] ||
              ...
              {
                id: 'tdp150',
                label: lang === 'en' ? '150W~180W Daily / Budget' : '150W~180W 甜点日常',
                matcher: (i) => {
                  const raw =
                    i.specs['标称解热功耗 (D-TDP)'] ||
                    i.specs['解热能力 (D-TDP)'] ||
      ...
      case 'psu':
        return [
          {
            id: 'wattage',
            label: lang === 'en' ? 'Rated Wattage' : '额定功率',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Wattages' : '全部功率', matcher: () => true },
              {
                id: '1000w',
                label: lang === 'en' ? '1000W+ Flagship Monster' : '1000W+ 旗舰怪兽',
                matcher: (i) => {
              ...
              {
                id: '850w',
                label: lang === 'en' ? '850W 3A Mainstream Sweetspot' : '850W 主流 3A 甜点',
                matcher: (i) => {
                  const s = (i.name + ' ' + (i.specs['额定功率'] || '')).toUpperCase();
                  return (
                    s.includes('850W') ||
              ...
              {
                id: '750w',
                label: lang === 'en' ? '650W~750W Budget Sweetspot' : '650W~750W 性价比',
                matcher: (i) => {
                  const s = (i.name + ' ' + (i.specs['额定功率'] || '')).toUpperCase();
                  return (
                    s.includes('750W') ||
          ...
          {
            id: 'standard',
            label: lang === 'en' ? 'Power Standard' : '标准规范',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Standards' : '全部规范', matcher: () => true },
              {
                id: 'atx3',
                label: lang === 'en' ? 'Native ATX 3.0 / 3.1 (12V-2x6)' : '原生 ATX 3.0 / 3.1 (12V-2x6)',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'atx2',
                label: lang === 'en' ? 'Classic ATX 2.x' : '经典 ATX 2.x',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
      ...
      case 'case':
        return [
          {
            id: 'structure',
            label: lang === 'en' ? 'Case Structure' : '机箱结构',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Structures' : '全部结构', matcher: () => true },
              {
                id: 'panoramic',
                label: lang === 'en' ? '270° Panoramic Dual-Chamber' : '270°全景无立柱海景房',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'mid-tower',
                label: lang === 'en' ? 'Mid-Tower Standard & Quiet' : '中塔标准风道/静音木纹',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
              ...
              {
                id: 'compact',
                label: lang === 'en' ? 'Compact M-ATX / ITX' : '紧凑便携 M-ATX / ITX',
                matcher: (i) => {
                  const s = (
                    i.name +
                    ' ' +
      ...
      default:
        return [
          {
            id: 'quickTag',
            label: lang === 'en' ? 'Featured Picks' : '装机精选',
            options: [
              { id: 'all', label: lang === 'en' ? 'All Hardware' : '全部硬件', matcher: () => true },
              {
                id: 'x3d-combo',
                label: lang === 'en' ? '9800X3D / 7800X3D Match' : '9800X3D / 7800X3D 绝配',
                matcher: (i) =>
              ...
              {
                id: 'rtx4070-combo',
                label: lang === 'en' ? 'RTX 4070 / 4070S Match' : 'RTX 4070 / 4070S 甜点搭档',
                matcher: (i) =>
              ...
              {
                id: 'white-theme',
                label: lang === 'en' ? 'Pure White Aesthetic' : '纯白颜值海景房',
                matcher: (i) =>
                  i.name.includes('白色') ||
                  i.name.includes('吹雪') ||
                  i.name.includes('海景房') ||
              ...
              {
                id: 'value',
                label: lang === 'en' ? 'Best Value Hits' : '高性价比爆款',
                matcher: (i) =>
        ...
        <>
          ...
          <div className="space-y-4 p-5 rounded-3xl bg-white dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
              ...
              <div className="relative flex-1">
                ...
                {searchQuery && (
                  <button
                    ...
                    title={lang === 'en' ? 'Clear search' : '清空搜索'}
                  >
              ...
              {/* Right Controls: Sort Selector & Segmented View Switcher */}
              <div className="flex items-center justify-between sm:justify-end space-x-2.5 shrink-0 flex-wrap gap-y-2">
                {/* Sort Selector */}
                <div className="flex items-center space-x-1.5">
                  ...
                  >
                    ...
                    <option value="price-desc">{t('sortPriceDesc') || (lang === 'en' ? 'Price: High to Low' : '价格：从高到低')}</option>
                    <option value="tdp">{t('sortTdp') || (lang === 'en' ? 'TDP: High to Low' : '功耗：TDP 从大到小')}</option>
                  </select>
                </div>

                {/* Vercel-Style Segmented View Switcher */}
                <div className="flex items-center bg-slate-100 dark:bg-neutral-800/90 p-1 rounded-xl border border-slate-200/80 dark:border-neutral-700/80 shrink-0">
                  <button
                    type="button"
            ...
            <div className="flex items-center space-x-2 overflow-x-auto pt-1 pb-1 scrollbar-none border-t border-slate-100 dark:border-slate-800/80">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 shrink-0 min-w-[76px] flex items-center">
                ...
                {t('brandFilter') || (lang === 'en' ? 'Brand Filter:' : '品牌筛选:')}
              </span>
                          ...
                          return (
                            <button
                              ...
                              title={
                                isZeroUnderBrand
                                  ? (lang === 'en'
                                      ? 'No hardware with this spec under selected brand. Click to switch to "All Brands".'
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="text-slate-400 text-[11px] font-medium mr-1">
                    {t('activeFiltersLabel') || (lang === 'en' ? 'Active Filters:' : '当前筛选条件:')}
                  </span>
                  ...
                  {selectedBrand !== 'all' && (
                    <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-cyan-400 border border-blue-200/60 dark:border-blue-900 font-medium">
                      <span>{lang === 'en' ? `Brand: ${selectedBrand}` : `品牌: ${selectedBrand}`}</span>
                      <button
                  ...
                  {searchQuery.trim() && (
                    <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 border border-amber-200/60 dark:border-amber-900 font-medium">
                      <span>{lang === 'en' ? `Search: "${searchQuery.trim()}"` : `搜索: "${searchQuery.trim()}"`}</span>
                      <button
                        onClick={() => setSearchQuery('')}
                  ...
                  <span className="text-slate-500 dark:text-slate-400 text-[11px] font-mono ml-1">
                    {t('foundHardwareCount', { count: filteredItems.length }) || (lang === 'en' ? `(${filteredItems.length} hardware items found)` : `(共找到 ${filteredItems.length} 款硬件)`)}
                  </span>
                </div>
                ...
                >
                  ...
                  <span>{t('resetAllFilters') || (lang === 'en' ? 'Reset All Filters' : '重置全部筛选')}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center space-x-2 overflow-x-auto py-1 scrollbar-none">
            ...
            {quickTerms.map((term) => (
              <button
                ...
                title={lang === 'en' ? `Click to view technical definition for "${term.term}"` : `点击查看「${term.term}」详细技术名词解释`}
              >
          ...
          {filteredItems.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-3 p-6">
              ...
              <h3 className="text-base font-bold text-slate-800 dark:text-white">
                {t('noHardwareFound') || (lang === 'en' ? 'No matching hardware found' : '未找到匹配条件的硬件型号')}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                {lang === 'en'
                  ? `Category: ${selectedCategory}${selectedBrand !== 'all' ? ` · Brand: ${selectedBrand}` : ''}${searchQuery ? ` · Search: "${searchQuery}"` : ''}. Try adjusting filters or searching different keywords.`
                  : `当前分类: ${selectedCategory}${selectedBrand !== 'all' ? ` · 品牌: ${selectedBrand}` : ''}${searchQuery ? ` · 搜索: "${searchQuery}"` : ''}，建议尝试调整筛选标签或更换关键词。`}
              </p>
              <button
                onClick={resetAllFilters}
              ...
              >
                <RotateCcw className="w-3.5 h-3.5" />
                <span>{t('resetFiltersAndShowAll') || (lang === 'en' ? 'Reset all filters' : '一键清除全部筛选并查看全部硬件')}</span>
              </button>
            </div>
          ) : viewMode === 'table' ? (
            <HardwareTableView
      ...
      {selectionTooltip && (
        ...
        >
          ...
          >
            ...
            <span>{lang === 'en' ? `Explain "${selectionTooltip.term.term.split(' ')[0]}"` : `解读「${selectionTooltip.term.term.split(' ')[0]}」`}</span>
          </button>
        </div>
      )}
