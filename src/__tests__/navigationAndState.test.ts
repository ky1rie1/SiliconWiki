...
describe('Navigation, URL Computation & History Synchronization Suite', () => {
  ...
  describe('History Semantics: In-App Modal vs Direct Share Link Entry', () => {
    it('should use history.back() when closing modal opened in-app to prevent duplicate /wiki history entries', () => {
      const win = (globalThis as unknown as { window: { history: { back: () => void; replaceState: () => void } } }).window;
      let hasInternalDetailPush = true;

      const handleCloseDetail = () => {
        if (hasInternalDetailPush) {
    ...
    it('should use replaceState when closing modal opened via direct share link to avoid exiting the site', () => {
      const win = (globalThis as unknown as { window: { history: { back: () => void; replaceState: (data: unknown, unused: string, url: string) => void } } }).window;
      let hasInternalDetailPush = false;

      const handleCloseDetail = (currentUrl: string) => {
        if (hasInternalDetailPush) {
          hasInternalDetailPush = false;
          win.history.back();
        } else {
          const targetUrl = computeDetailCloseUrl(currentUrl);
          win.history.replaceState(null, '', targetUrl);
        }
      };

      handleCloseDetail('https://computer-wiki.vercel.app/?hardware=cpu-amd-9800x3d');
      expect(win.history.back).not.toHaveBeenCalled();
      expect(win.history.replaceState).toHaveBeenCalledWith(null, '', '/#/wiki');
    });

    it('should synchronize category and hardware state on popstate/hashchange event', () => {
      let currentCategory = 'all';
      let currentDetailId: string | null = null;

      const syncFromUrl = (url: string) => {
        currentCategory = parseCategoryFromUrl(url);
        currentDetailId = parseHardwareFromUrl(url);
      };

      // User presses browser Forward to ?category=gpu&hardware=gpu-rtx-4070-super
      syncFromUrl('https://computer-wiki.vercel.app/?category=gpu&hardware=gpu-rtx-4070-super#/wiki');
      expect(currentCategory).toBe('gpu');
      expect(currentDetailId).toBe('gpu-rtx-4070-super');

      // User presses browser Back to ?category=cpu
      syncFromUrl('https://computer-wiki.vercel.app/?category=cpu#/wiki');
      expect(currentCategory).toBe('cpu');
      expect(currentDetailId).toBeNull();
    });
  });

  describe('StrictMode Single History Push Contract', () => {
    it('should ensure state updater function remains pure while side-effects live in event handlers', () => {
      const win = (globalThis as unknown as { window: { history: { pushState: (data: unknown, unused: string, url: string) => void } } }).window;

      // In React StrictMode, updater functions (prev => ...) are invoked twice in development.
      // Side-effects like history.pushState must NOT be placed inside the updater function.
      let state = 'wiki';
      const handleTabChange = (newTab: string) => {
        // Correct pattern: pushState in handler
        win.history.pushState({ tab: newTab }, '', `/#/${newTab}`);
        // Pure updater function (simulated StrictMode double invocation)
        const updater = (_current: string) => newTab;
        state = updater(updater(state));
      };

      handleTabChange('rankings');
      expect(state).toBe('rankings');
      // Must be called exactly once despite updater running twice
